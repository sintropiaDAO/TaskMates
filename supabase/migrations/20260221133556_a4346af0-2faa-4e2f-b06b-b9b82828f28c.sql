...
CREATE POLICY "Users can update own like" ON public.comment_likes FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete own like" ON public.comment_likes FOR DELETE USING (auth.uid() = user_id);
