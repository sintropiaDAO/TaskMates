...
export function ShareTaskButton({ taskId, taskTitle }: ShareTaskButtonProps) {
  const { user } = useAuth();
  ...
  const taskUrl = getSharePreviewUrl('task', taskId);
  ...
  const inviteMessage = language === 'pt'
    ...
    : `🤝 Hi! I'd like to invite you to collaborate on the task "${taskTitle}" on TaskMates — a collaboration platform based on task and resource exchange. Join us!\n\n${taskUrl}`;
  ...
  const handleInviteUser = async (userId: string) => {
    ...
    const message = language === 'pt'
      ? `🤝 Você foi convidado para colaborar na tarefa "${taskTitle}"`
      : `🤝 You've been invited to collaborate on task "${taskTitle}"`;
        ...
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {language === 'pt' ? 'Compartilhar Tarefa' : 'Share Task'}
            </DialogTitle>
          </DialogHeader>

          <div className="space-y-4">
            {/* Invite user from app */}
