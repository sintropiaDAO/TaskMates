...
import {
  ...
  ShoppingCart, Truck, Eye, EyeOff, Users as UsersIcon, Lock, Copy
} from 'lucide-react';
...
export function ProductDetailModal({
  ...
  const [deliveryMode, setDeliveryMode] = useState<'code' | 'proof'>('code');
  ...
  const fetchComments = async () => {
    if (!product) return;
    // We reuse task_comments table logic but for products we'll use a simple approach
    // For now, comments are not stored — this is a placeholder for the chat section
    setComments([]);
  };
  ...
  const canConfirmDelivery = isOwner && (hasSuppliers || hasRequesters) && !isDelivered;
  ...
  const handleConfirmDelivery = async () => {
    ...
    if (deliveryMode === 'proof') {
      ...
      await supabase
        ...
        .update({
          delivery_confirmed: true,
          delivery_code_input: deliveryCode,
        })
        .eq('product_id', product.id)
        .eq('user_id', user.id);
    }
  ...
  const handleStartGroupChat = async () => {
    ...
    // Find or create a conversation for this product's participants
    const participantIds = [...new Set(participants.map(p => p.user_id))];
    if (participantIds.length < 2) return;

    // For now, open a direct chat with the first non-self participant
  ...
  const copyDeliveryCode = () => {
    if (product?.delivery_code) {
      navigator.clipboard.writeText(product.delivery_code);
      toast({ title: language === 'pt' ? 'Código copiado!' : 'Code copied!' });
    }
  };
      ...
      <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto p-0">
          <div className="p-4 sm:p-6 space-y-5">
            ...
            {/* Delivery code for owner/requester */}
            {isOwner && product.delivery_code && (
              <div className="glass rounded-lg p-3 border border-border/50">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">
                      {language === 'pt' ? 'Código de entrega (envie ao solicitador)' : 'Delivery code (send to requester)'}
                    </p>
                    <p className="text-2xl font-mono font-bold tracking-widest">{product.delivery_code}</p>
                  </div>
            ...
            {canConfirmDelivery && (
              <Button className="w-full gap-2" variant="default" onClick={() => setShowDeliveryModal(true)}>
            ...
            <Collapsible defaultOpen={participants.length > 0}>
              <CollapsibleTrigger className="flex items-center justify-between w-full py-2 text-sm font-semibold hover:text-primary transition-colors">
                <span className="flex items-center gap-2">
                  ...
                  {language === 'pt' ? 'Pessoas Envolvidas' : 'Participants'} ({participants.filter(p => p.user_id !== product.created_by).length})
                </span>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`text-xs px-1.5 py-0.5 rounded ${
                            p.role === 'supplier' ? 'bg-success/10 text-success' : 'bg-pink-600/10 text-pink-600'
                          }`}>
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button
                size="sm"
                variant={deliveryMode === 'code' ? 'default' : 'outline'}
                className="flex-1 text-xs"
                onClick={() => setDeliveryMode('code')}
              >
              ...
              <Button
                size="sm"
                variant={deliveryMode === 'proof' ? 'default' : 'outline'}
                className="flex-1 text-xs"
                onClick={() => setDeliveryMode('proof')}
              >
                <Image className="w-3 h-3 mr-1" />
                {language === 'pt' ? 'Foto/Link' : 'Photo/Link'}
              </Button>
            </div>
            ...
            {deliveryMode === 'code' ? (
              <div className="space-y-2">
                <Label className="text-sm">
                  {language === 'pt' ? 'Insira o código de 4 dígitos' : 'Enter the 4-digit code'}
                </Label>
                <div className="flex justify-center">
                  <InputOTP maxLength={4} value={deliveryCode} onChange={setDeliveryCode}>
                    <InputOTPGroup>
                ...
                {proofMode === 'file' ? (
                  <div>
                    ...
                    <Button variant="outline" className="w-full text-xs" onClick={() => fileInputRef.current?.click()}>
                      <Upload className="w-3 h-3 mr-1" />
                      {proofFile ? proofFile.name : (language === 'pt' ? 'Selecionar foto' : 'Select photo')}
                    </Button>
                  </div>
                ) : (
                  <Input
                    placeholder={language === 'pt' ? 'Cole o link aqui' : 'Paste link here'}
                    value={proofUrl}
            ...
            <Button
              ...
              disabled={
                confirming ||
                (deliveryMode === 'code' && deliveryCode.length !== 4) ||
                (deliveryMode === 'proof' && proofMode === 'file' && !proofFile) ||
                (deliveryMode === 'proof' && proofMode === 'link' && !proofUrl.trim())
              }
